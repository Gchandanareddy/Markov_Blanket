...
# Claude AI grading upgrade ---------------------------------------------------
# Add your key in Streamlit secrets as:
# ANTHROPIC_API_KEY = "sk-ant-..."
# Or set it as an environment variable before running Streamlit.

QUESTION_RUBRICS = {
    "q1": {
        "question": "What is the Markov Blanket of Y?",
        "ideal": "MB(Y) = {W, X, Z, T}. Parents of Y are W and X; children are Z and T; spouses/co-parents include W, X, and Z, so the unique Markov blanket is W, X, Z, T.",
        "rubric": "Correct if the student lists W, X, Z, and T or clearly explains parents, children, and spouses for Y. Partial if they include at least three correct nodes or show the right Markov blanket idea but miss one node. Incorrect if they include mostly wrong nodes or misunderstand the concept.",
    },
    "q2": {
        "question": "What is the Markov Blanket of W?",
        "ideal": "MB(W) = {X, Y, Z}. W has no parents; children are Y and Z; X is a co-parent/spouse through Y and Z.",
        "rubric": "Correct if the student lists X, Y, and Z or explains children plus co-parent. Partial if they include two correct nodes or have the concept but miss one node. Incorrect if they mostly miss the answer.",
    },
    "q3": {
        "question": "What are the parents in the Markov Blanket of Z?",
        "ideal": "Parents of Z are W, X, and Y because W→Z, X→Z, and Y→Z.",
        "rubric": "Correct if the student lists W, X, and Y as parents of Z. Partial if they include two of the three. Incorrect if they do not identify the direct parents.",
    },
    "q4": {
        "question": "If we regress Z on X, W, Y, not T, which variables will be statistically significant?",
        "ideal": "W, X, and Y will be statistically significant because all three are direct parents of Z.",
        "rubric": "Correct if the student says W, X, and Y are significant and preferably explains they are direct parents of Z. Partial if they mention two of the three or generally say the parents are significant. Incorrect if they choose T or misunderstand the regression relationship.",
    },
    "q5": {
        "question": "Which variables/arcs should be stratified or adjusted for to measure the un-confounded effect of Y on T?",
        "ideal": "Condition/stratify on W and X to block the backdoor paths Y←W→Z→T and Y←X→Z→T. Do not condition on Z if estimating total effect because Z is a mediator on Y→Z→T.",
        "rubric": "Correct if the student identifies W and X as the adjustment set and does not incorrectly insist on conditioning on Z as necessary for total effect. Partial if they identify only W or only X, or mention backdoor/confounding but incomplete. Incorrect if they condition mainly on Z as the answer or miss W and X.",
    },
}
...
def grade_with_claude(q_key, student_answer):
    """Grade with Claude when available; otherwise use the original rule-based grader."""
    fallback_grade = FALLBACK_GRADERS[q_key](student_answer)
    fallback_feedback = get_feedback(q_key, fallback_grade, student_answer)

    api_key = get_anthropic_key()
    if not api_key or requests is None:
        return {"grade": fallback_grade, "feedback": fallback_feedback, "source": "fallback"}

    rubric = QUESTION_RUBRICS[q_key]
    prompt = f"""
You are grading a Health Informatics student's answer about DAGs, Markov blankets, regression, and causal adjustment.
Grade generously but accurately. Accept equivalent phrasing, not just exact node listing.

Question: {rubric['question']}
Ideal answer: {rubric['ideal']}
...
{{
  ...
  "feedback": "A short, supportive 2-4 sentence tutoring response. Do not give the next question."
}}
""".strip()
    ...
    try:
        ...
        grade = str(result.get("grade", fallback_grade)).lower().strip()
        if grade not in {"correct", "partial", "incorrect"}:
            grade = fallback_grade
        feedback = clean_ai_feedback(result.get("feedback", "")) or fallback_feedback
        return {"grade": grade, "feedback": feedback, "source": "claude"}
    except Exception:
        return {"grade": fallback_grade, "feedback": fallback_feedback, "source": "fallback"}
...
Type **restart** to try again! 🔄""")
    ...
    if s in q_map:
        qkey = q_map[s]
        result = grade_with_claude(qkey, txt)
        grade = result["grade"]
